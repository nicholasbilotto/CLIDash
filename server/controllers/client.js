...
export const getContracts = async (req, res) => {
	try {
		...
		if (globalFilter) {
			...
			const globalQuery = {
				$or: [
					{ Licensor: { $regex: regex } },
					{ Licensee: { $regex: regex } },
		...
		// Fetch the contracts with pagination, sorting, and filtering
		const contracts = await Contract.find(query)
			.sort(parsedSort)
			.skip(skip)
			.limit(pageSizeNum);

		// Count the total number of filtered contracts
		const total = await Contract.countDocuments(query);

		// Send back the contracts and the total count
		res.status(200).json({ docs: contracts, totalDocs: total });
	} catch (error) {
